...
public class MainInterFace extends BaseTabs {
    ...
    public MainInterFace(Stage stage) { // initialize main interface components
        ...
        image = new Image(Objects.requireNonNull(MainInterFace.class.getResource("/library.jpg")).toExternalForm(), 650, 500, true, true);
    ...
    private void actions() { // method for main interface components actions
        ...
        read.setOnAction(e -> { // read from file chosen by file chooser
            ...
            try {
                File initialDirectory = ApplicationFiles.sampleBooksFile().getParent().toFile();
                if (initialDirectory.isDirectory()) {
                    fc.setInitialDirectory(initialDirectory);
                }
            } catch (IOException ex) {
                Handler.alert("Error", "Unable to prepare application data folder", Alert.AlertType.ERROR);
            }
